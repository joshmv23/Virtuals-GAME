...
const aw_contracts_sdk_1 = require("@lit-protocol/aw-contracts-sdk");
const wrapped_key_1 = require("./wrapped-key");
...
class Admin {
    ...
    static savePkpsToStorage(storage, adminAddress, pkps) {
        ...
        if (!admins[adminAddress]) {
            admins[adminAddress] = { privateKey: '', pkps: [], wrappedKeys: [] };
        }
     ...
     */
    async getWrappedKeys() {
        return (0, wrapped_key_1.loadWrappedKeysFromStorage)(this.storage);
    }
    /**
     ...
     */
    async getWrappedKeyById(id) {
        const wrappedKey = (0, wrapped_key_1.loadWrappedKeyFromStorage)(this.storage, id);
        if (!wrappedKey) {
            throw new errors_1.AwSignerError(errors_1.AwSignerErrorType.ADMIN_WRAPPED_KEY_NOT_FOUND, `Wrapped key with id ${id} not found in storage`);
        }
        return wrappedKey;
    }
    /**
     ...
     * @throws If the wrapped key is not found.
     */
    async removeWrappedKey(id) {
        const wrappedKey = await this.getWrappedKeyById(id);
        (0, wrapped_key_1.removeWrappedKeyFromStorage)(this.storage, id);
        return wrappedKey;
    }
    /**
     ...
     */
    async mintWrappedKey(pkpTokenId) {
        const pkp = await this.getPkpByTokenId(pkpTokenId);
        const authMethod = await lit_auth_client_1.EthWalletProvider.authenticate({
        ...
        return (0, wrapped_key_1.mintWrappedKey)(this.litNodeClient, pkpSessionSigs, pkp.info.tokenId, this.litNetwork, this.storage);
    }
    /**
    ...
    static async create(adminConfig, { litNetwork, debug = false } = {}) {
        ...
        const toolRegistryConfig = aw_contracts_sdk_1.DEFAULT_REGISTRY_CONFIG[litNetwork];
        ...
        return new Admin(storage, litNetwork, litNodeClient, litContracts, (0, aw_contracts_sdk_1.getPkpToolRegistryContract)(toolRegistryConfig, adminWallet), adminWallet);
    }
    /**
    ...
    async getRegisteredToolsAndDelegateesForPkp(pkpTokenId) {
        ...
        const registeredTools = await (0, aw_contracts_sdk_1.getRegisteredToolsAndDelegatees)(this.toolRegistryContract, this.litContracts, (await this.getPkpByTokenId(pkpTokenId)).info.tokenId);
