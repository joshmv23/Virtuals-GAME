...
(async () => {
    ...
    const policyParameters = await (0, aw_tool_1.getPolicyParameters)(pkpToolRegistryContract, pkpTokenId, parentToolIpfsCid, delegateeAddress, ['maxAmount', 'allowedTokens', 'allowedRecipients']);
    let maxAmount;
    let allowedTokens = [];
    ...
    for (const parameter of policyParameters) {
        ...
        switch (parameter.name) {
            ...
            case 'allowedRecipients':
                ...
                allowedRecipients = allowedRecipients.map((addr) => ethers.utils.getAddress(addr));
                console.log(`Formatted allowedRecipients: ${allowedRecipients.join(', ')}`);
                break;
        }
    }
    ...
    if (allowedTokens.length > 0) {
        ...
        if (!allowedTokens.includes(ethers.utils.getAddress(tokenInfo.tokenAddress))) {
            throw new Error(`Token ${tokenInfo.tokenAddress} not allowed. Allowed tokens: ${allowedTokens.join(', ')}`);
        }
    }
    if (allowedRecipients.length > 0) {
        console.log(`Checking if ${tokenInfo.recipientAddress} is an allowed recipient...`);
        if (!allowedRecipients.includes(ethers.utils.getAddress(tokenInfo.recipientAddress))) {
            throw new Error(`Recipient ${tokenInfo.recipientAddress} not allowed. Allowed recipients: ${allowedRecipients.join(', ')}`);
        }
    }
