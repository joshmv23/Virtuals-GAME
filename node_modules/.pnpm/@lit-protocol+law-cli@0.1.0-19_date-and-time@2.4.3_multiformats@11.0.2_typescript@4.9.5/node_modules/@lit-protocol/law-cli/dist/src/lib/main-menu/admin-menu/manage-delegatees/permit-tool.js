...
exports.handlePermitToolForDelegatee = void 0;
const tslib_1 = require("tslib");
const prompts_1 = tslib_1.__importDefault(require("prompts"));
const core_1 = require("../../../core");
const promptSelectDelegateeToPermitToolFor = async (delegatees) => {
    if (delegatees.length === 0) {
        throw new core_1.LawCliError(core_1.PermitToolForDelegateeErrors.NO_DELEGATEES_FOUND, 'No delegatees found for PKP.');
    }
    ...
    if (!delegatee) {
        throw new core_1.LawCliError(core_1.PermitToolForDelegateeErrors.PERMIT_TOOL_FOR_DELEGATEE_CANCELLED, 'Permit tool for delegatee cancelled.');
    }
    return delegatee;
};
const promptSelectToolToPermit = async (registeredTools, permittedTools) => {
    ...
    if (registeredTools === null ||
        ...
        throw new core_1.LawCliError(core_1.PermitToolForDelegateeErrors.NO_TOOLS_FOUND, 'No unpermitted tools found.');
    }
    const choices = [
...
const handlePermitToolForDelegatee = async (admin, pkp) => {
    try {
        ...
        if (registeredTools === null ||
            (Object.keys(registeredTools.toolsWithPolicies).length === 0 &&
                Object.keys(registeredTools.toolsWithoutPolicies).length === 0 &&
                Object.keys(registeredTools.toolsUnknownWithPolicies).length === 0)) {
            throw new core_1.LawCliError(core_1.PermitToolForDelegateeErrors.NO_TOOLS_FOUND, 'No tools found.');
        }
        const delegatee = await promptSelectDelegateeToPermitToolFor(await admin.awAdmin.getDelegatees(pkp.info.tokenId));
        const selectedTool = await promptSelectToolToPermit(registeredTools, await admin.awAdmin.getPermittedToolsForDelegatee(pkp.info.tokenId, delegatee));
        await admin.awAdmin.permitToolForDelegatee(pkp.info.tokenId, selectedTool.ipfsCid, delegatee);
        core_1.logger.success(`${selectedTool.name} successfully permitted for delegatee ${delegatee}.`);
    }
    catch (error) {
        if (error instanceof core_1.LawCliError) {
            if (error.type === core_1.PermitToolForDelegateeErrors.NO_TOOLS_FOUND ||
                error.type === core_1.PermitToolForDelegateeErrors.NO_DELEGATEES_FOUND ||
                error.type ===
                    core_1.PermitToolForDelegateeErrors.PERMIT_TOOL_FOR_DELEGATEE_CANCELLED) {
...
exports.handlePermitToolForDelegatee = handlePermitToolForDelegatee;
